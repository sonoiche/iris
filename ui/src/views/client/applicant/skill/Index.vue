                    ...
                    <div class="card-title w-100">
                        <div class="d-flex justify-content-between w-100">
                            <div class="d-flex align-items-center">
                                <h3 class="fw-bolder m-0">Manage Applicant Skills</h3>
                            </div>
                            <div class="d-flex align-items-center">
                                <button class="btn btn-primary btn-sm" @click="addSkill">Add Skill</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-striped table-hover w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th class="fw-bolder">Skill Name</th>
                            ...
                            <tbody v-if="skills.length">
                                <tr v-for="(skill, index) in skills" :key="skill">
                                    ...
                                    <td class="align-middle">{{ skill.skill_level_name }}</td>
                                    <td class="align-middle">{{ skill.remarks }}</td>
                                    <td class="text-center">
                                        ...
                                        <div class="dropdown-menu menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                            <div class="menu-item px-3">
                                                <a href="javascript:;" class="menu-link px-3" @click="editSkill(skill.id)">Edit</a>
                                            </div>
                                            <div class="menu-item px-3">
                                                <a href="javascript:;" class="menu-link px-3" @click="deleteSkill(skill.id)">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
export default {
    setup(props, {emit}) {
        ...
        const { status, skills, getSkills, destroySkill } = skillRepo();

        const addSkill = () => {
            emit('add-data', 'ApplicantCreateSkill');
        }

        const editSkill = (id) => {
            emit('add-data', 'ApplicantEditSkill', id);
        }
        ...
        onMounted( async () => {
            await getSkills(route.params.id);
            state.isLoading = false;
        });
