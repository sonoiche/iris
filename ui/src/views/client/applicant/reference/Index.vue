                    ...
                    <div class="card-title w-100">
                        <div class="d-flex justify-content-between w-100">
                            <div class="d-flex align-items-center">
                                <h3 class="fw-bolder m-0">Manage Applicant References</h3>
                            </div>
                            <div class="d-flex align-items-center">
                                <button class="btn btn-primary btn-sm" @click="addReference">Add Reference</button>
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <div class="dropdown-menu menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                            <div class="menu-item px-3">
                                                <a href="javascript:;" class="menu-link px-3" @click="editReference(reference.id)">Edit</a>
                                            </div>
                                            <div class="menu-item px-3">
                                                <a href="javascript:;" class="menu-link px-3" @click="deleteReference(reference.id)">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
export default {
    setup(props, {emit}) {
        ...
        const { status, references, getReferences, destroyReference } = referenceRepo();

        const addReference = () => {
            emit('add-data', 'ApplicantCreateReference');
        }

        const editReference = (id) => {
            emit('add-data', 'ApplicantEditReference', id);
        }
        ...
        onMounted( async () => {
            await getReferences(route.params.id);
            state.isLoading = false;
        });
