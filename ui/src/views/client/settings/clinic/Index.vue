<template>
    <div>
                        ...
                        <div class="card mb-5 mb-xl-12">
                            <div class="card-header border-0 d-flex justify-content-between">
                                <div class="card-title">
                                    <h3 class="fw-bolder m-0">Clinic Manager</h3>
                                </div>
                                <div class="d-flex align-items-center" v-if="isCanWrite('Clinic Manager')">
                                    <button class="btn btn-primary btn-sm" @click="addClinic">Add Clinic</button>
                                </div>
                            </div>
                            <div class="card-body border-top p-9">
                                <div class="d-flex justify-content-end">
                                    <div class="d-flex align-items-center position-relative my-1">
                                        ...
                                        <input type="text" class="form-control form-control-solid w-250px ps-14" v-model="page.search_table" @keyup="searchDatatable" placeholder="Search Clinic Manager" />
                                    </div>
                                </div>
                                <table class="table align-middle table-row-dashed fs-6 gy-5" id="clinic-table">
                                    <thead>
                                        <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                            <th class="w-10px pe-2">#</th>
                                            <th class="min-w-125px">Clinic Name</th>
        ...
        <ModalClinic :is-active="modalActive" :clinic="clinic" :isLoading="page.isLoading" @close-modal="closeModalClinic" @refresh-table="refresh" />
    </div>
</template>
...
import ModalClinic from '@/views/client/settings/clinic/modals/Index.vue';
...
export default {
    setup() {
        ...
        const { status, clinic, getClinic, destroyClinic } = clinicRepo();
        ...
        const initDatatable = (result = {}) => {
            ...
            $('#clinic-table').DataTable({
                ...
                'serverSide': true,
                ajax: {
                    url: `${process.env.VUE_APP_API_ENDPOINT}/client/clinics/datatable`,
                    type: 'POST',
                    data: {
                ...
                'columns': [
                    ...
                    { 'data': 'contact_number', searchable: true, orderable: true },
                    { 'data': 'contact_person', searchable: true, orderable: true },
                    { 'data': 'action', searchable: false, orderable: false, className: "text-center",
                        render: function(data, type, row, meta) {
                            if(isCanWrite('Clinic Manager') && isCanDelete('Clinic Manager')) {
                                return `
        ...
        const removeClinic = (id) => {
            ...
            swal({
                ...
                if (result.isConfirmed) {
                    await destroyClinic(page.idToDelete);
                    if(status.value == 200) {
                        $('#clinic-table').DataTable().destroy();
                        initDatatable();
                    }
        ...
        onMounted(() => {
            ...
            $('tbody', '#clinic-table').on( 'click', '.edit-status', function(){
                const cell =  $('#clinic-table').DataTable().cell( $(this).closest("td") );
                editClinic(cell.data());
            });

            $('tbody', '#clinic-table').on( 'click', '.remove-status', function(){
                const cell =  $('#clinic-table').DataTable().cell( $(this).closest("td") );
                removeClinic(cell.data());
            });
        });
