<template>
    <div>
                        ...
                        <div class="card mb-5 mb-xl-12">
                            <div class="card-header border-0 d-flex justify-content-between">
                                <div class="card-title">
                                    <h3 class="fw-bolder m-0">Applicant Sources</h3>
                                </div>
                                <div class="d-flex align-items-center" v-if="isCanWrite('Applicant Source')">
                            ...
                            <div class="card-body border-top p-9">
                                <div class="d-flex justify-content-end">
                                    <div class="d-flex align-items-center position-relative my-1">
                                        ...
                                        <input type="text" class="form-control form-control-solid w-250px ps-14" v-model="page.search_table" @keyup="searchDatatable" placeholder="Search sources" />
                                    </div>
                                </div>
                                <table class="table align-middle table-row-dashed fs-6 gy-5" id="source-table">
                                    <thead>
        ...
        <ModalSource :is-active="modalActive" :source="source" :isLoading="page.isLoading" @close-modal="closeModalSource" @refresh-table="refresh" />
    </div>
</template>
...
import ModalSource from '@/views/client/settings/sources/modals/Index.vue';
...
export default {
    setup() {
        ...
        const { status, source, getSource, destroySource } = sourceRepo();
        ...
        const initDatatable = (result = {}) => {
            ...
            $('#source-table').DataTable({
                ...
                'serverSide': true,
                ajax: {
                    url: `${process.env.VUE_APP_API_ENDPOINT}/client/sources/datatable`,
                    type: 'POST',
                    data: {
                ...
                'columns': [
                    ...
                    { 'data': 'action', searchable: false, orderable: false, className: "text-center",
                        render: function(data, type, row, meta) {
                            if(isCanWrite('Applicant Source') && isCanDelete('Applicant Source')) {
                                return `
                                ...
                                <div class="dropdown-menu menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                    <div class="menu-item px-3">
                                        <a href="javascript:;" class="menu-link px-3 edit-source">Edit</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <a href="javascript:;" class="menu-link px-3 remove-source">Delete</a>
                                    </div>
                                </div>
                                `
                            }
        ...
        const removeSource = (id) => {
            ...
            swal({
                ...
                if (result.isConfirmed) {
                    await destroySource(page.idToDelete);
                    if(status.value == 200) {
                        $('#source-table').DataTable().destroy();
                        initDatatable();
                    }
        ...
        onMounted(() => {
            ...
            $('tbody', '#source-table').on( 'click', '.edit-source', function(){
                const cell =  $('#source-table').DataTable().cell( $(this).closest("td") );
                editSource(cell.data());
            });

            $('tbody', '#source-table').on( 'click', '.remove-source', function(){
                const cell =  $('#source-table').DataTable().cell( $(this).closest("td") );
                removeSource(cell.data());
            });
        });
